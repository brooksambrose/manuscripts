---
title: "Development of Scholarly Disciplines"
...
ev=F
ec=F
knitr::opts_chunk$set(eval=ev,echo=ev)
...
```{r t-process,eval=T,echo=ec}
t$process<-data.frame(matrix(letters[1:4],nrow=2,ncol=2,dimnames=list(Scale=c('Micro','Macro'),Mechanism=c('Cultural','Social'))))
sg(t$process,lab='t-process',title='Process',col.align = c('cc','ll'))
```

```{r t-stages,eval=ev,echo=ec}
t$stages<-data.frame(
...
sg(t$stages,lab='t-stages',title='Stages',col.align = c('ccc','lll'))
```
...
```{r t-relevance,eval=T,echo=ec}
sg(data.frame(t$relevance),lab='t-relevance',title='Relevance',col.align = c('cc','ll'))
```
...
```{r t-prefix,eval=T,echo=ec}
gng2tts<-gng2tts.f()
t$prefix<-dcast(unique(gng2tts$ts[,list(Phrase,stem,cat)]),stem~cat,value.var='Phrase',fun=paste,collapse=', ')
t$prefix[c('anth','psyc'),'A.generic':=ital(A.generic),with=F]
...
```{r f-prefix,eval=T,echo=ec,fig.height=10,fig.width=7.5,fig.cap='Disciplinary Prefixes',fig.label='f-prefix'}
# #chunk options
gng2tts$p
```
...
```{r master2wok-f,eval=T,echo=ec,include=F}
dpfx<-c('ANTH', 'SOCI', 'ECON', 'POLI','PSYC')
if(any(grepl('master2wok.RData',dir('resources')))) {
	load('resources/master2wok.RData')
	master2wok[,usa:=as.logical(ifelse(grepl(" USA,",PA),T,F))]
	setkey(master2wok,usa)
	master2wok[list(T),Prefix:=sapply(SO,tag)]
} else {
	...
	save(master2wok,file='master2wok.RData')
}
```

```{r,eval=T,echo=ec,include=F}
...
m2w[is.na(Prefix),Prefix:='Non-english out-sample'] %>% setkey(Prefix)
m2w['Out',Prefix:='Non-prefix out-sample'] %>% setkey(Prefix)
setorder(m2w,-N)
```
...
```{r}
sg(m2w,lab = 'Journals in the Web of Knowledge Master List',col.align = c(old='cccc',new='lrrl'))
```
...
```{r dbl2inspect-f,eval=ev,echo=ec}
system.time(x<-dbl2inspect.f(wok2dbl))
```

```{r wok2dbl-cleaning,eval=ev,echo=ec}
##DROP
...
```{r dbl2dbw-f,eval=ev,echo=ec}
attributes(wok2dbl)$col.ord<-col.ord
dbl2dbw<-dbl2dbw.f(wok2dbl)
```

```{r dbl2dbw-imputing,eval=ev,echo=ec}
...
```{r dbl2dbw-deriv,eval=ev,echo=ec}
...
```{r wok2dbl-summary1,eval=ev,echo=ec}
graphics.off()
setkey(dbl2dbw,PY)
...
```{r jstor-wok-comparison,eval=ev,echo=ec}
setkey(jstor2dbw,SO)
setkey(dbl2dbw,SO)
