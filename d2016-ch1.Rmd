...
ev=F
ec=F
knitr::opts_chunk$set(eval=ev,echo=ev,fig.align='center',comment=NA,results='asis')
...
stargazer(
	data.frame(matrix(letters[1:4],nrow=2,ncol=2,dimnames=list(Scale=c('Micro','Macro'),Mechanism=c('Cultural','Social'))))
	,summary=F
	,title='Ontology'
	,style='ajs'
	,type='latex',header=F
...
Consider four social science disciplines--anthropology, economics, psychology, and sociology. In english the flag uniting the concept of discipline are the prefixes anth-, econ-, psyc-, and soci-. We contend that these prefixes will diffuse first as generic and weakly categorical terms ()

#TODO reshape data.table for nice table of flagbearers
```{r t-dprefixes,eval=T,echo=ec}
gng2tts<-gng2tts.f()
sg(gng2tts$ts[,.N,by=c('stem','cat','Phrase')],title = 'Flagbearers',col.align=c(old='cccc',new='llll'))
```

```{r f-disciplines,eval=T,echo=ec,fig.height=10,fig.width=7.5,fig.cap='Disciplinary Prefixes'}
# #chunk options
gng2tts$p
```
...
```{r master2wok-f,eval=ev,echo=ec}
