...
```{r mstr2mstr,eval=T,include=F}
jstorm<-fread(rev(dir(pattern='JSTOR_Global_AllTitlesArchive',recursive=T))[1])
...
if(any(grepl('master2wok.RData',dir('resources')))) {
	...
	master2wok[,Prefix:=sapply(SO,tag)]
} else {
...
load('../mstr2mstr/m2m.RData')
setkey(master2wok,ix)
master2wok[list(m2m$wix),`:=`(jix=m2m$jix,jstor=jstorm$publication_title[m2m$jix])]
setkey(master2wok,Prefix)
```

Between the `r try(nrow(jstorm))` items on the JSTOR master list and the `r try(nrow(master2wok))` on the WOK master list, `r try(nrow(m2m))` overlap. This defines a sampling frame of journals for which full citation data and limited text data are available. If we filter further on journals published in the United States, the frame is reduced to `r try(nrow(master2wok[!is.na(jix)][]))`.
...
```{r f-prefix,eval=T,fig.height=10,fig.width=7.5,fig.cap='Disciplinary Prefixes',fig.label='f-prefix'}
# #chunk options
gng2tts$p
```
...
```{r master2wok-f, eval=T, include=F}
master2wok[,`:=`(j=!is.na(jix),p=Prefix!='Out')]

```

```{r,eval=T,include=T, message=FALSE, warning=FALSE, include=T}
#sg(
